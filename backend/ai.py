...
UNDERLINE_PROMPT = """You are an expert competitive debate coach specializing in "card tracing" — the practice of underlining the most important text in debate evidence cards.
...
HIGHLIGHT_PROMPT = """You are an expert competitive debate coach. You will receive a list of underlined passages from a debate evidence card. Your task is to select 15-25% of the underlined text for highlighting.
...
async def underline_card(card: dict, topic: str, prompt: str) -> tuple[dict, str]:
    """
    Returns (result_dict, model_id_used).
    Fallback across models is handled by the router.
    """
    user_msg = (
        f"TOPIC: {topic}\n\n"
        f"CARD TAG: {card['tag']}\n"
    ...
    try:
        text, model_id = await router.call(prompt, user_msg, max_tokens=2048)
        json_match = re.search(r"\{[\s\S]*\}", text)
        if json_match:
            return json.loads(json_match.group(0)), model_id
        return {"relevant": False, "reason": "No JSON in response", "underlined": []}, model_id
    except json.JSONDecodeError:
        return {"relevant": False, "reason": "JSON parse error", "underlined": []}, "none"
    except Exception as exc:
        return {"relevant": False, "reason": str(exc), "underlined": []}, "none"


async def highlight_card(card: dict, underlined: list, prompt: str) -> tuple[dict, str]:
    """
    Returns (result_dict, model_id_used).
    """
    ...
    try:
        text, model_id = await router.call(prompt, user_msg, max_tokens=1024)
        json_match = re.search(r"\{[\s\S]*\}", text)
        if json_match:
            return json.loads(json_match.group(0)), model_id
        return {"highlighted": []}, model_id
    except json.JSONDecodeError:
